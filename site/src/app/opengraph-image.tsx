...
export default async function Image() {
	...
	return new ImageResponse(
		(
			<div style={{ background: '#0c1417', width: '100%', height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', padding: '72px 80px', fontFamily: 'Inter, sans-serif' }}>
				<span style={{ fontSize: 13, letterSpacing: '0.18em', color: 'rgba(255,255,255,0.3)', textTransform: 'uppercase' }}>vf-clamp</span>
				<div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
						...
						{[
							...
							return (
								<div key={label} style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
									<span style={{ fontSize: 11, color: 'rgba(255,255,255,0.25)', fontFamily: 'monospace', width: 32 }}>{label}</span>
									<div style={{ position: 'relative', width: w, height: 4, background: 'rgba(255,255,255,0.08)', borderRadius: 2, display: 'flex' }}>
					...
					<div style={{ fontSize: 76, color: '#ffffff', lineHeight: 1.06, fontWeight: 300 }}>Restrict the range,</div>
					<div style={{ fontSize: 76, color: 'rgba(255,255,255,0.38)', lineHeight: 1.06, fontWeight: 300, fontStyle: 'italic' }}>keep what varies.</div>
				</div>
